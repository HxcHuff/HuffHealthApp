...
import {
  ...
  type HopperConversationChannel,
} from "@/lib/conversation-channel";

interface ConversationListItem {
  id: string;
  twilioConversationSid: string;
  channel: HopperConversationChannel;
...
export function ConversationsWorkspace({ initialConversations, initialUnreadCount }: Props) {
  ...
  const [channelFilter, setChannelFilter] = useState<"" | HopperConversationChannel>("");
  ...
  const isMessenger = details?.conversation.channel === "FACEBOOK_MESSENGER";
  ...
  const consentBadge = (() => {
    if (isMessenger) {
      return { color: "bg-blue-100 text-blue-800", label: "Facebook Messenger", icon: MessageSquare };
    }
  ...
  const segmentCount = !isMessenger && composerBody ? smsSegmentCount(composerBody) : 0;
        ...
        <header className="border-b border-gray-200 p-3">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
            ...
            <select
              ...
              value={channelFilter}
              onChange={(e) =>
                setChannelFilter(e.target.value as "" | HopperConversationChannel)
              }
            >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-1 flex items-center gap-2 text-[10px]">
                      <span className="rounded bg-gray-100 px-1.5 py-0.5 font-medium text-gray-700">
                        {conversationChannelLabel(c.channel)}
                      </span>
                      {c.unreadCount > 0 ? (
            ...
            <footer className="border-t border-gray-200 p-3">
              ...
              (consentBadge?.label === "No consent on file" ||
                ...
                <div className="mb-2 flex items-center gap-2 rounded-md bg-red-50 px-3 py-2 text-xs text-red-800">
                  ...
                  <span>
                    No valid TCPA consent on file. Outbound SMS will be blocked by compliance.
                  </span>
                </div>
              ) : null}
              {isMessenger ? (
                <div className="mb-2 flex items-center gap-2 rounded-md bg-blue-50 px-3 py-2 text-xs text-blue-800">
                  ...
                  <span>
                    Messenger thread — replies stay on Facebook. No SMS fallback.
                  </span>
                </div>
              ) : null}
              {insuranceIsMedicare ? (
