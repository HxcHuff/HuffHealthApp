...
export async function POST(req: NextRequest) {
  ...
  const limit = checkRateLimit({
    key: `twilio-messenger-status:${ip}`,
    limit: 1200,
    windowMs: 60_000,
  });
  if (!limit.ok) {
  ...
  if (process.env.NODE_ENV === "production") {
    ...
    if (!valid) {
      console.warn("[twilio-messenger-status] Invalid Twilio signature");
  ...
  void (async () => {
    try {
      await processMessageStatus({
        messageSid,
        status,
        errorCode: body.ErrorCode || body.ChannelStatusMessage || null,
        errorMessage: body.ErrorMessage || body.ChannelStatusMessage || null,
      });
    } catch (err) {
      console.error("[twilio-messenger-status] async processing error", err);
    }
  })();
